...
def objectDetection(frame, option):
    ...
    for contour in contours:
        ...
        if w * h > minim:
            landingFound = True
            flag += 1
            print(f"Jumlah terdeteksi: {flag}, Area: {areaLuas},  {option}")
            x1 = int(((x + x + w) / 2))
            y1 = int(((y + y + h) / 2))
            area += int(w * h)
    ...
    if landingFound:
        ...
        return cogXFinal, cogYFinal, frame, dilationMask
    else:
        cogXFinal = -1
        cogYFinal = -1
        return cogXFinal, cogYFinal, frame, dilationMask
...
def calculateFPS(fps, dist, action, frame, centerPos, errorPos):
    ...
    cv.putText(frame, f"Error: {errorPos}", (1, 30), cv.FONT_HERSHEY_SIMPLEX, 0.3, (0, 0, 0))
